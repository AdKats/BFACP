    ...
    <table class="table table-striped table-condensed" ng-table="tableParams" show-filter="true">
        <tbody>
            <tr ng-repeat="(key, session) in $data">
                ...
                <td class="hidden-sm hidden-md" ng-bind="session.Headshots | number" sortable="'Headshots'" data-title="'<?php echo Lang::get('player.profile.stats.server.table.col6'); ?>'"></td>
                <td ng-bind="session.Deaths | number" sortable="'Deaths'" data-title="'<?php echo Lang::get('player.profile.stats.server.table.col7'); ?>'"></td>
                <td class="hidden-sm hidden-md" ng-bind="session.Suicide | number" sortable="'Suicide'" data-title="'<?php echo Lang::get('player.profile.stats.server.table.col8'); ?>'"></td>
                <td class="hidden-sm hidden-md" ng-bind="session.TKs | number" sortable="'TKs'" data-title="'<?php echo Lang::get('player.profile.stats.server.table.col9'); ?>'"></td>
                ...
                <td class="hidden-sm hidden-md" ng-bind="session.RoundCount | number" sortable="'RoundCount'" data-title="'<?php echo Lang::get('player.profile.stats.server.table.col11'); ?>'"></td>
                <td class="hidden-sm hidden-md" ng-bind="session.Wins | number" sortable="'Wins'" data-title="'<?php echo Lang::get('player.profile.stats.server.table.col14'); ?>'"></td>
                <td class="hidden-sm hidden-md" ng-bind="session.Losses | number" sortable="'Losses'" data-title="'<?php echo Lang::get('player.profile.stats.server.table.col15'); ?>'"></td>
