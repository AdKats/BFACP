<alert type="success" ng-if="weapons.acs.length === 0 && !refresh.acs && !weapons.acsError"><i class="fa fa-check"></i>&nbsp;No
    Suspicious Weapons Found
</alert>
<alert type="info" ng-if="weapons.acs.length === 0 && refresh.acs && !weapons.acsError">
    <i class="fa fa-spinner fa-pulse"></i>&nbsp;Checking player&hellip;</alert>
<alert type="error" ng-if="!refresh.acs && weapons.acsError"><i class="fa fa-times"></i>&nbsp;{{ weapons.acsErrorMsg }}
</alert>
...
<div class="table-responsive" ng-if="weapons.acs.length > 0">
    <table class="table table-striped table-condensed">
        <thead>
        <th width="250px">
            <span class="pull-left"><?php echo Lang::get('player.profile.acs.table.col1'); ?></span><span class="pull-right"><?php echo Lang::get('player.profile.acs.table.col2'); ?></span>
        </th>
        ...
        <tr ng-repeat="(key, weapon) in weapons.acs | orderBy:'kills':true">
            ...
            <td ng-class="{'bg-green': !weapon.triggered.DPS, 'bg-red': weapon.triggered.DPS}">
                <span ng-if="weapon.triggered.DPS">&plus;</span>{{ weapon.dps }}%
            </td>
